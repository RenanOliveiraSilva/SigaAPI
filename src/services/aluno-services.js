...
export async function getStudentData(cookiesInput) {
    if (!cookiesInput.length)
        return null;
    ...
    try {
        ...
        const frame = await waitForFrameBy(page, (f) => /siga/i.test(f.url()), 60_000);
        if (!frame)
        ...
        const dados = await frame.evaluate(() => {
            const headers = Array.from(document.querySelectorAll("#GRID .uc_appinfo-header")).map((el) => el.textContent?.trim()) || null;
            const content = Array.from(document.querySelectorAll("#GRID .uc_appinfo-content")).map((el) => el.textContent?.trim()) || null;
            // Extrair dados a partir do conteúdo
            const contentText = content[0] || "";
            const extractedData = {
                name: headers[0] || "",
                courseStatus: contentText.split("Status:")[1]?.split("RA:")[0]?.trim() || "",
                ra: contentText.split("RA:")[1]?.split("Unidade:")[0]?.trim() || "",
                college: contentText.split("Unidade:")[1]?.split("Curso:")[0]?.trim() || "",
                curse: contentText.split("Curso:")[1]?.split("Turno:")[0]?.trim() || "",
                turne: contentText.split("Turno:")[1]?.split("PP:")[0]?.trim() || "",
                semester: parseInt(contentText.split("Semestre:")[1]?.split("Máximo:")[0]?.trim()) || 0,
            };
...
async function waitForFrameBy(page, match, timeoutMs = 30_000, pollMs = 250) {
